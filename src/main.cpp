...
// Sends door/window opening state
...
void sleep()
{
  ...
  sleep_mode();
  sleep_disable();
  detachInterrupt(SENSOR_DATA_PIN);
}
...
void setup()
{
  ...
  digitalWrite(SENSOR_DATA_PIN, HIGH);
...
void loop()
{
  ...
  int sensorValue = digitalRead(SENSOR_DATA_PIN);
